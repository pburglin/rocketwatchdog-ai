...
program
  ...
  .action(async (options) => {
    ...
    if (!ctx.decision) {
      ...
      console.log(
        JSON.stringify(
          { workload: workload.id, workloadTrace: ctx.workloadTrace, decision: guardResult.decision },
          null,
          2
        )
      );
    ...
    console.log(
      JSON.stringify(
        { workload: workload.id, workloadTrace: ctx.workloadTrace, decision: ctx.decision },
        null,
        2
      )
    );

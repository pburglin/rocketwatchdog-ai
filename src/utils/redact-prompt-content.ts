...
export function redactPromptBearingContent<T>(value: T, patterns: string[]): { redacted: T; changed: boolean } {
  const result = redactValue(value, patterns, false, 0);
  return { redacted: result.value as T, changed: result.changed };
}
...
function redactValue(
  ...
  if (typeof value === "string") {
    if (!active) return { value, changed: false };
  ...
  let changed = false;
  const redactedRecord: Record<string, unknown> = { ...record };

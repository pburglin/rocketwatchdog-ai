...
export class OutputGuardsStage {
    async run(ctx) {
        if (!ctx.policy)
        ...
        const result = runGuards({ text: response, ...(toolInvocations ? { toolInvocations } : {}) }, ctx.policy, ctx.snapshot.platform, ctx.snapshot.toolSchemas);

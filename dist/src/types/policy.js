export function mergeEffectivePolicy(platform, workload) {
    ...
    const maxPrompt = clampNumber(policy.max_prompt_chars ?? platform.security.max_prompt_chars ?? 0, platform.security.max_prompt_chars);
    const maxOutput = clampNumber(policy.max_output_chars ?? platform.security.max_output_chars ?? 0, platform.security.max_output_chars);
    return {
...
function clampNumber(value, cap) {
    if (!cap || cap <= 0)
        return value;
    if (value <= 0)
        return cap;
...
function defaultInputGuards(level) {
    if (level === "L3") {
        return {
...
function defaultOutputGuards(level) {
    if (level === "L3") {
...
function defaultToolGuards(level) {
    if (level === "L3") {
        return {

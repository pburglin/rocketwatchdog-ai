...
describe("validateTools", () => {
    ...
    it("flags missing tool schema when validation is required", () => {
        ...
        const result = validateTools(policyWithMissing, [{ name: "allowed" }], [{ name: "missing", arguments: { id: "1" } }], toolSchemas);
        expect(result.allowed).toBe(false);
        expect(result.reasons[0]).toContain("TOOL_SCHEMA_MISSING");
    });
    it("validates tool invocation schema", () => {
        const result = validateTools(policy, [{ name: "allowed" }], [{ name: "allowed", arguments: {} }], toolSchemas);
        expect(result.allowed).toBe(false);
        expect(result.reasons[0]).toContain("TOOL_SCHEMA_INVALID");
    });
    it("accepts valid tool invocation", () => {
        const result = validateTools(policy, [{ name: "allowed" }], [{ name: "allowed", arguments: { id: "123" } }], toolSchemas);
        expect(result.allowed).toBe(true);
    });
    it("rejects tool usage when allowlist is empty", () => {
        const result = validateTools({ ...policy, allowed_tools: [] }, [{ name: "allowed" }], [{ name: "allowed", arguments: { id: "123" } }], toolSchemas);
        expect(result.allowed).toBe(false);

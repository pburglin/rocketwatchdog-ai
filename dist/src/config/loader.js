...
function validateSnapshot(platform, workloads, toolSchemas) {
    ...
    for (const workload of workloads) {
        ...
        if (allowedBackends.length > 0) {
            ...
            if (missing.length > 0) {
                throw new Error(`Workload ${workload.id} references unknown llm_backends: ${missing.join(", ")}`);
            }
        }
        if (allowedMcpBackends.length > 0) {
            const missing = allowedMcpBackends.filter((name) => !platform.mcp_backends[name]);
            if (missing.length > 0) {
                throw new Error(`Workload ${workload.id} references unknown mcp_backends: ${missing.join(", ")}`);
            }
        }
        const llmDuplicates = findDuplicates(policy.allowed_llm_backends);
        if (llmDuplicates.length > 0) {
            throw new Error(`Workload ${workload.id} has duplicate allowed_llm_backends: ${llmDuplicates.join(", ")}`);
        }
        const mcpDuplicates = findDuplicates(policy.allowed_mcp_backends);
        if (mcpDuplicates.length > 0) {
            throw new Error(`Workload ${workload.id} has duplicate allowed_mcp_backends: ${mcpDuplicates.join(", ")}`);
        }
        if (policy.allowed_models?.length) {
            const modelDuplicates = findDuplicates(policy.allowed_models);
            if (modelDuplicates.length > 0) {
                throw new Error(`Workload ${workload.id} has duplicate allowed_models: ${modelDuplicates.join(", ")}`);
            }
            ...
            if (missingModels.length > 0) {
                throw new Error(`Workload ${workload.id} references unknown allowed_models: ${missingModels.join(", ")}`);
            }
        }
        if (policy.allowed_tools?.length) {
            const duplicates = findDuplicates(policy.allowed_tools);
            if (duplicates.length > 0) {
                throw new Error(`Workload ${workload.id} has duplicate allowed_tools: ${duplicates.join(", ")}`);
            }
        }
        if (guards?.tools?.require_tool_schema_validation) {
            if (Object.keys(toolSchemas).length === 0) {
                throw new Error(`Workload ${workload.id} requires tool schemas but configs/tools is empty`);
            }
            if (policy.allowed_tools?.length) {
                const missingSchemas = policy.allowed_tools.filter((tool) => !toolSchemas[tool]);
                if (missingSchemas.length > 0) {
                    throw new Error(`Workload ${workload.id} requires tool schemas but missing: ${missingSchemas.join(", ")}`);
                }
            }
        }
    }

...
export function explainWorkloadResolution(platform, workloads, ctx) {
    const steps = [];
    ...
    if (metaPath) {
        ...
        if (!(allowOverride || (sourceApp && trustedApps.has(sourceApp)))) {
            steps.push({
                ...
                detail: typeof metaValue === "string"
    ...
    for (const workload of workloads) {
        ...
        if (match.headers) {
            const mismatchedHeader = Object.entries(match.headers).find(([key, value]) => ctx.headers[key.toLowerCase()] !== value);
            if (mismatchedHeader) {
                steps.push({
                    stage: "match_rules",
                    outcome: "miss",
        ...
        if (match.metadata) {
            const mismatchedMetadata = Object.entries(match.metadata).find(([key, value]) => getByJsonPath(ctx.payload ?? {}, key) !== value);
